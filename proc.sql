...
RETURNS TABLE(emp_id int, name text) AS $$
	...
	LOOP
		FETCH curs INTO r;
		IF (r.job_type = 'part_time_instructor') THEN
			SELECT count(*) INTO total_hours
			FROM Sessions
			WHERE instructor = r.eid AND EXTRACT(month FROM session_date) = EXTRACT(month FROM date);
		...
		IF (exists(
			SELECT 1
			FROM Sessions
			WHERE instructor = r.eid AND date = session_date AND start_time >= session_hour - 1 AND start_time <= session_hour + 1)) THEN
			CONTINUE;
		END IF;
		emp_id := r.eid;
		name := r.name;
