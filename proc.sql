...
CREATE TYPE employee_status AS ENUM ('full-time', 'part-time');
CREATE TYPE employee_category AS ENUM ('administrator', 'manager', 'instructor');
CREATE OR REPLACE PROCEDURE add_employee(name text, address text, phone text, email text, emp_status employee_status, salary int, join_date date, emp_category employee_category, course_areas text[]) AS $$
...
RETURNS TABLE(emp_id int, name text, total_hours int, day date, avail_hours int[]) AS $$
...
CREATE TYPE payment_method AS ENUM ('credit-card', 'course-package');
