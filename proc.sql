...
CREATE OR REPLACE FUNCTION sessions_clash(start1 int, duration1 int, start2 int, duration2 int) RETURNS BOOLEAN AS $$
BEGIN
	RETURN ranges_overlap(start1, start1 + duration1, start2, start2 + duration2);
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION instructor_sessions_clash(start1 int, duration1 int, start2 int, duration2 int) RETURNS boolean AS $$
DECLARE
	break_time int;
BEGIN
	break_time := 1;
	RETURN ranges_overlap(start1, start1 + duration1 + break_time, start2, start2 + duration2 + break_time);
END;
$$ LANGUAGE plpgsql;
...
BEGIN
	...
	LOOP
		...
		LOOP
			...
			month_hours := instructor_month_hours(curr_emp.eid, EXTRACT(year FROM curr_date), EXTRACT(month FROM curr_date));
